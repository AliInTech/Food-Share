...
const Dashboard = () => {
  ...
  const fetchFood = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get('https://food-share-backend-whd6.onrender.com/api/food', { 
  ...
  const handleClaim = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://food-share-backend-whd6.onrender.com/api/food/claim/${id}`, {}, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
  ...
  const handleAcceptSubmit = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://food-share-backend-whd6.onrender.com/api/food/confirm-donation/${selectedFood._id}`, formData, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
      setAcceptOpen(false);
  ...
  const handleFinalVerify = async (otpValue) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://food-share-backend-whd6.onrender.com/api/food/verify-otp`, 
  ...
  const handleConfirmPurge = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://food-share-backend-whd6.onrender.com/api/food/${itemToDelete._id}`, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
